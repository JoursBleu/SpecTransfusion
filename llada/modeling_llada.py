...
class LLaDAModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        self.dummy_embeddings = nn.Parameter(
            torch.randn(config.spec_depth, config.d_model, device=config.init_device)
        )
    ...
    def device(self) -> torch.device:
        device: torch.device = self.dummy_embeddings.weight.device  # type: ignore
        if device.type == "meta":
    ...
    def forward(
        ...
        if past_key_values:
            ...
            start_kv = len(past_key_values) - self.config.n_layers
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                layer_past = None if past_key_values is None else past_key_values[start_kv+block_idx]
                if (
                    (self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.whole_layer)
                    or (
                        self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.one_in_two
